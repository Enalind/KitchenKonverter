...
describe("Utilities", () => {
    test("Decimal to fraction", () => {
        ...
        expect(decimalToFraction(5, "ml" ,decimalToFractionLookup)).toEqual([5, "ml"])
        expect(decimalToFraction(3.12, "l" ,decimalToFractionLookup)).toEqual(["3 and 1/10", "l"])
    })
    test("Convert decimal and unit to fraction", () => {
        const unitList = conversions["volume"]["metric"]
        expect(calculateUnitString(50, unitList, decimalToFractionLookup)).toEqual([5, "cl"])
        expect(calculateUnitString(432.12, unitList, decimalToFractionLookup)).toEqual(["4 and 1/3", "dl"])
    })
})